...
class TextEditTestReplication(unittest.TestCase):
    ...
    def test_deleting_entire_fragment_conflicts_with_insert_of_single_fragment(self):
        ...
        self.assertEqual(test2.text.get_text(), "abcxyzghi")
        non_deleted_list_fragments = [f for f in test2.text._listfragments if f.text != ""]
        deleted_list_fragments = [f for f in test2.text._listfragments if f.id == deleted_middle_fragment.id]
        # There can be either one or two fragments because of the ordering of events
        self.assertTrue(len(deleted_list_fragments) == 1 or len(deleted_list_fragments) == 2)
        # Check they really were deleted
        self.assertTrue(all([f.text == "" for f in deleted_list_fragments]))
    ...
    def test_deleting_multiple_fragment_conflicts_with_insert_of_single_fragment(self):
        ...
        textowner.text.insert(0, "abcghi")
        textowner.text.insert(3, "def")
        textowner.text.insert(3, "jkl")
        self.assertEqual(textowner.text.get_text(), "abcjkldefghi")
        self.assertEqual(len(textowner.text._listfragments), 4)

        deleted_middle_fragments = textowner.text._listfragments[1:2]

        test2 = self.dc2.get_object_by_id(TestFieldTextEditOwner1.__name__,
                                          textowner.id)

        self.dc2.freeze_dc_comms()
        textowner.text.removerange(3,9)
        test2.text.insert(6,"xyz")
        self.dc2.unfreeze_dc_comms()

        self.assertEqual(textowner.text.get_text(), "abcxyzghi")
        non_deleted_list_fragments = [f for f in textowner.text._listfragments if f.text != ""]
        deleted_list_fragments = [f for f in textowner.text._listfragments if f.id in {f2.id for f2 in deleted_middle_fragments}]
        # Check they really were deleted
        self.assertTrue(all([f.text == "" for f in deleted_list_fragments]))
        self.assertEqual(len(non_deleted_list_fragments), 3)
        fragments = non_deleted_list_fragments
        ...
        assert fragments[1].text == "xyz"
        assert fragments[1].relative_to == deleted_list_fragments[0].id
        ...
        self.assertEqual(test2.text.get_text(), "abcxyzghi")
        non_deleted_list_fragments = [f for f in test2.text._listfragments if f.text != ""]
        deleted_list_fragments = [f for f in test2.text._listfragments if f.id in {f2.id for f2 in deleted_middle_fragments}]
        # There can be either one or two fragments because of the ordering of events
        self.assertTrue(len(deleted_list_fragments) == 1 or len(deleted_list_fragments) == 2)
        # Check they really were deleted
        self.assertTrue(all([f.text == "" for f in deleted_list_fragments]))
